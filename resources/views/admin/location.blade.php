                ...
                <tr class="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td class="p-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <button type="button" class="edit-btn w-8 h-8 flex items-center cursor-pointer justify-center text-[#A89070] hover:bg-slate-50 hover:text-[#8E795E] rounded-md transition-colors"
                                onclick="editLocation({{ $location->id }})" title="Edit">
                                <i class="bi bi-pencil-square"></i>
                            </button>
                            
                            @role('super_admin')
                            <button type="button" class="delete-btn w-8 h-8 flex items-center cursor-pointer justify-center text-red-400 hover:bg-red-50 hover:text-red-600 rounded-md transition-colors"
...
<div id="location-modal" class="fixed inset-0 z-[100] hidden items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm opacity-0 transition-opacity duration-300">
    <div class="bg-white rounded-3xl shadow-2xl border border-slate-100 w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden transform scale-95 transition-transform duration-300">
        <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            ...
            <button type="button" onclick="closeModal()" class="text-slate-400 cursor-pointer hover:text-slate-600 transition-colors">
        ...
        <form id="location-form" class="flex flex-col min-h-0" enctype="multipart/form-data">
            <div class="overflow-y-auto p-6 space-y-5">
                ...
                <div>
                    ...
                    <input type="file" name="hero_image" id="hero_image" accept="image/*" class="w-full cursor-pointer rounded-xl border-slate-200 px-4 py-2 border focus:ring-primary focus:border-primary transition-colors bg-white">
                    <p class="text-sm text-slate-500 mt-1">Max size: 5MB.</p>
                </div>
            ...
            <div class="p-6 border-t border-slate-100 flex justify-end gap-3 bg-slate-50/50 shrink-0">
                ...
                <button type="submit" id="submit-btn" class="w-full cursor-pointer sm:w-auto bg-primary text-white px-6 py-2.5 rounded-xl font-medium hover:bg-[#8E795E] transition-all shadow-sm">Save Location</button>
            </div>
        </form>
    </div>
</div>
